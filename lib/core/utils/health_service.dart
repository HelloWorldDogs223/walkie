...
class HealthService {
    ...
    try {
      ...
      if (healthData.isNotEmpty) {
        ...
        for (var point in healthData) {
          print(
            'HealthService: 데이터 포인트: ${point.value}, 소스: ${point.sourceName}',
          );
          if (point.value is NumericHealthValue) {
            int pointSteps = (point.value as NumericHealthValue).numericValue
                .toInt();
            steps += pointSteps;
        ...
        if (Platform.isAndroid) {
          print(
            'HealthService: Android에서 Samsung Health 또는 Google Fit 연동이 필요할 수 있음',
          );
        }
      }
    } catch (e) {
  ...
  static void _initAccelerometer() {
    _accelerometerSubscription = accelerometerEventStream().listen((event) {
      ...
      final avgYAxis =
          _yAxisHistory.reduce((a, b) => a + b) / _yAxisHistory.length;

      final timeDiff = now.difference(_lastStepTime).inMilliseconds;
      final peakTimeDiff = now
          .difference(_lastPeakCandidateTime)
          .inMilliseconds;

      // 플랫폼별 민감도 설정 (더 민감하게 조정)
      final bool isIOS = Platform.isIOS;
      final threshold = isIOS ? 9.5 : 10.2; // 임계값 낮춤 (더 민감하게)
      final minInterval = isIOS ? 300 : 360; // 최소 간격 줄임 (더 빠른 감지)
      final yAxisThreshold = isIOS ? 1.5 : 2.2; // Y축 임계값 낮춤

      // 직접적인 피크 감지 + 필터링된 피크 감지 (두 방식 병행)
      bool isDirectPeak =
          yWeightedMagnitude > threshold &&
          yWeightedMagnitude > _previousFilteredMagnitude + 1.0 && // 변화량 요구 완화
      ...
      if (isDirectPeak || isFilteredPeak) {
        ...
        if (peakTimeDiff > 30) {
          ...
          if (magnitudeChange > 0.8) {
            ...
            print(
              'HealthService: [$platformInfo] 가속도계 걸음 감지! (${isDirectPeak ? '직접' : '필터'}, 임계값: ${threshold.toStringAsFixed(1)}, 크기: ${(isDirectPeak ? yWeightedMagnitude : avgMagnitude).toStringAsFixed(1)}) 총 $_quickStepCount 걸음',
            );
          }
        }
      }
  ...
  static Future<int> calculateTodaySteps(int currentTotalSteps) async {
    ...
    if (todaySteps > 50000) {
      ...
      if (healthSteps != null && healthSteps < todaySteps) {
        print(
          'HealthService: 비정상 값 감지, Health API로 보정: $todaySteps -> $healthSteps',
        );
        return healthSteps;
      }
    }
    ...
    print(
      'HealthService: 오늘 걸음수: $todaySteps (총: $currentTotalSteps, 기준: $todayBaseSteps)',
    );
    return todaySteps;
  }
